...
const MOCK_PRODUCTS_MAP: Record<string, any> = {
  "wireless-headphones": {
    ...
    description: "Experience absolute acoustic bliss. Equipped with hybrid active noise cancellation, premium memory foam earcups, and high-fidelity dynamic drivers. Enjoy up to 40 hours of playtime on a single charge.",
    price: 45000,
    discountPrice: 38000,
    images: [
  ...
  "classic-wristwatch": {
    ...
    description: "Crafted for timeless style and precision. Featuring a stainless steel case, sapphire glass casing protection, and Japanese quartz chronograph movement. Water-resistant up to 50 meters.",
    price: 65000,
  ...
  "ergonomic-office-chair": {
    ...
    description: "Optimize your work setup for complete comfort. Built with breathable mesh backs, dynamic 3D armrests, and premium pneumatic height adjustment. Reduces spine strain during long working sessions.",
    price: 95000,
    discountPrice: 80000,
    images: [{ url: "https://images.unsplash.com/photo-1505797149-43b0069ec26b?w=800&auto=format&fit=crop&q=80" }],
    avgRating: 4.7,
    reviewCount: 30,
    stock: 8,
    specifications: {
  ...
  "smart-fitness-tracker": {
    ...
    description: "Keep track of your health metrics dynamically. Monitors heart rate, blood oxygen levels, and active sleep states in real-time. Syncs seamlessly with iOS and Android devices.",
    price: 25000,
    images: [{ url: "https://images.unsplash.com/photo-1579586337278-3befd40fd17a?w=800&auto=format&fit=crop&q=80" }],
    avgRating: 4.4,
    reviewCount: 9,
    stock: 0,
    specifications: {
...
async function getProductBySlug(slug: string) {
  ...
  return MOCK_PRODUCTS_MAP[slug] || null;
}
        ...
        <section className="border-t border-border pt-10 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-surface-secondary border border-border p-6 rounded-2xl">
            {Object.entries(product.specifications).map(([key, val]) => (
              <div key={key} className="flex justify-between py-2 border-b border-border/40 last:border-b-0 text-sm">
                <span className="font-semibold text-muted-foreground">{key}</span>
                <span className="font-bold text-foreground text-right">{val as string}</span>
              </div>
            ))}
          </div>
        </section>
